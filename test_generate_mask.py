...
from dataset import (AdeSegmentationIncremental,
                     CityscapesSegmentationIncrementalDomain,
                     VOCSegmentationIncremental, transform,
                     EndoInstrumentSegmentationIncremental,
                     EndoInstrumentSegmentationIncremental_visualization)
...
def get_dataset(opts):
    ...
    #===========================#
        ...
        # dataset = EndoInstrumentSegmentationIncremental
        dataset = EndoInstrumentSegmentationIncremental_visualization
    #===========================#
    else:
    ...
    if opts.dataset == 'endo_instru':

        file_name_list = opts.file_name_list
        ...
        # print('len(train_dst)', len(train_dst))
        val_dst = dataset(
            root=opts.data_root,
            step=opts.step,
            train=False,
        ...
        # )
        test_dst = val_dst
        # print('len(test_dst)', len(test_dst))
...
def main(opts):
    ...
    for i, (step, lr) in enumerate(zip(copy.deepcopy(opts.step), copy.deepcopy(opts.lr))):
        ...
        opts.step = step # now opts.step has already been updated into "int" value
        opts.lr = lr
...
# ======== MY: run step actually means run task ============== #
def run_step(opts, world_size, rank, device):
    ...
    # MY: step in this code means task
    ...
    if opts.model == 'SwinUnet':
        params.append(
            {
                "params": filter(lambda p: p.requires_grad, model.feature_extractor.parameters()),
    ...
    if opts.model != 'SwinUnet':
        # ====== original one using DeepLab(ResNet101) model ====== #
    ...
    # ************************************************************************************* #
    # xxx Load old model from old weights if step > 0! # MY: if task > 0
    if opts.step > 0:
        # get model path
        if opts.step_ckpt is not None:
            ...
            path = f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step - 1}_best.pth" # load old best weights
    # ************************************************************************************* #
    ...
    if opts.step > 0 and step_checkpoint is not None:
        if 'trainer_state' in step_checkpoint:
            # print('mengyaaaaaaaaaaaaaaaaaaaaaaaaa') # Enter this step
            trainer_state = step_checkpoint['trainer_state']
            # print('trainer_state', trainer_state) # {'regularizer': None}
    ...
    if TRAIN:
        ...
        for cur_epoch in range(opts.epochs): 
            ...
            if (cur_epoch + 1) % opts.val_interval == 0:
                ...
                model.eval()
                val_loss, val_score, ret_samples = trainer.validate_visualization(
                    loader=val_loader,
                    metrics=val_metrics,
...
#                 model.eval()
#                 test_loss, test_score, _ = trainer.validate(
#                     loader=test_loader, metrics=val_metrics, logger=logger, end_task=True
#                 )

#                 logger.print("Done test")
#                 logger.info(
#                     f"End of test {cur_epoch}/{opts.epochs}, test Loss={test_loss[0]+test_loss[1]},"
#                     f" Class Loss={test_loss[0]}, Reg Loss={test_loss[1]}"
#                 )
#                 # ======================== MY: print all metrics ==================== #
#                 logger.info(val_metrics.to_str(test_score))
# # ================================================================================================ #

                # =====  Save Best Model (From author, something is wrong, not real best model）===== #
                if rank == 0:  # save best model at the last iteration
                    score = val_score['Mean IoU']
                    # best model to build incremental steps
                    # print('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&')
                    # print('trainer.state_dict()', trainer.state_dict())
                    save_ckpt(
                ...
                # ===================== MY: save real best model ======================== #
                if rank == 0:
                    iou = val_score['Mean IoU']
                    if iou > best_iou:
                        best_iou = iou
                        best_epoch = cur_epoch

                        save_ckpt(
                            f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step}_best.pth", model,
                            trainer, optimizer, scheduler, best_epoch, best_iou
                        )
                        # print('best_iou', best_iou)
                        logger.info("[!] Real Best Checkpoint saved.")

                print('#'*80)
                logger.info(
                    f"End of Validation {cur_epoch}/{opts.epochs}, best epoch={best_epoch},"
                    f" best iou={best_iou}"
                )
                # print('#'*80)
                # ======================================================================= #
    ...
    # make data loader https://pytorch.org/docs/stable/data.html
    test_loader = data.DataLoader(
        test_dst,
        batch_size=opts.batch_size if opts.crop_val else 1,
        sampler=DistributedSampler(test_dst, num_replicas=world_size, rank=rank),
    ...
    if True: #TRAIN:
        ...
        if opts.ckpt is not None and os.path.isfile(opts.ckpt):
            ...
            ckpt = f"{opts.checkpoint}/{task_name}_{opts.name}_{opts.step}_best.pth"
        # ================================================= #
    ...
    #     loader=test_loader, metrics=val_metrics, logger=logger, end_task=True
    # )
    val_loss, val_score, _ = trainer.validate_visualization(
        loader=test_loader, metrics=val_metrics, logger=logger, end_task=True
    )
...
if __name__ == '__main__':
    ...
    # ================= Added by my =============== #
    if opts.model == 'SwinUnet':
        opts.checkpoint = opts.checkpoint+"_"+opts.model
    # ================================================= #
    os.makedirs(f"{opts.checkpoint}", exist_ok=True)

    main(opts)

# https://www.whcsrl.com/blog/1016777