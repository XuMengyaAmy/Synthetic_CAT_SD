...
DATASET=endo_instru
TASK=17-18
NAME=ILT_and_Intermediate_Shift_248_24_loss_de_50_Custumized_T
METHOD=ILT_and_Intermediate_Shift_248_24_loss_de_50_Custumized_T
...
SAVED=./checkpoints/step_temperature_apply_customized_unique4.0_rest3.0_shift_trial_2
...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 32 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs 100 --method ${METHOD} --step_ckpt ${FIRSTMODEL} --checkpoint ${SAVED} --temperature_apply customized --temperature 4.0 --temperature_rest_class 3.0 --test
