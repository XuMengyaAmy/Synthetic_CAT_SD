...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} test_generate_mask.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 1 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 0 --lr 0.1 --epochs 100 --method ${METHOD} --model ${MODEL} --checkpoint=./checkpoints_Long_SwinUnet/step_0.1_0.1 --test
