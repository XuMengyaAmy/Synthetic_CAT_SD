...
DATASET=endo_instru
TASK=17-18
NAME=ILT_and_Intermediate_Shift_0_4_loss_de_50_T_35
METHOD=ILT_and_Intermediate_Shift_0_4_loss_de_50_T_35
...
SAVED=./checkpoints/step_real_temperature_apply_old
...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 32 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs 100 --method ${METHOD} --step_ckpt ${FIRSTMODEL} --checkpoint ${SAVED} --temperature_apply old --temperature 3.5
