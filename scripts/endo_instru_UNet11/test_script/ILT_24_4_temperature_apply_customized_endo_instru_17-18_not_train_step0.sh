...
DATASET=endo_instru
TASK=17-18
NAME=ILT_and_Intermediate_Shift_24_4_loss_de_50_Custumized_T
METHOD=ILT_and_Intermediate_Shift_24_4_loss_de_50_Custumized_T
MODEL=UNet11
...
CHECKPOINT=./checkpoints_UNet11/step_synthetic_temp_shift

# CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 32 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 0 --lr 0.01 --epochs 100 --method ${METHOD} --ckpt ${FIRSTMODEL} --test --model ${MODEL}
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 32 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs 100 --method ${METHOD} --step_ckpt ${FIRSTMODEL} --model ${MODEL} --checkpoint ${CHECKPOINT} --temperature_apply customized --temperature 3.0 --temperature_rest_class 4.0 --file_name_list train_17.txt test_17.txt train_harmonized_blended_18_GAN_17_memory500.txt test_17_18.txt --test --crop_val
