...
CUDA_VISIBLE_DEVICES=${GPU} python3 -m torch.distributed.launch --master_port ${PORT} --nproc_per_node=${NB_GPU} run.py --date ${START_DATE} --data_root ${DATA_ROOT} --batch_size 32 --dataset ${DATASET} --name ${NAME} --task ${TASK} --step 1 --lr 0.001 --epochs 100 --method ${METHOD} --step_ckpt ${FIRSTMODEL} --file_name_list train_17.txt test_17.txt train_harmonized_blended_18.txt test_17_18_robustness_impulse_noise.txt --checkpoint ${SAVED} --test
